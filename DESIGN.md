...
- **Text stops where its box is full (2026-09-26).** The board keeps its size at every setting, so the grid around it (the 96px header, the 402px side column, the 146px ticker row) and the waiting screens' 620px rail stay fixed too. Each fixed box re-sets the `--text-*` variables on itself with `min(factor, its maximum)` (`cappedTextScaleVars`), the maximums living in `tvTextScaleMax` (`design-tokens.ts`): the highest factor at which nothing leaves the box or runs into its padding at 1920×1080 with six players and long names. Main board and claiming header 1× (both are already full at the design — the claim counter touches the bottom edge), main board and claiming side column 1.3×, initial-placement header 1.4×, waiting title 1.7×, waiting rail 1.1×. The ticker fits the whole range and has no cap. Everything inside one box stops together; below 50 every box follows the setting. Two layout changes make the side column's 1.3× possible: the main board's army count and its "LEGERS" label sit at `leading-none` (a row is 84px instead of 105px, like the other large TV numbers — header counter, timer), and player names in both side columns are cut off with "…" (`truncate`), because a name has no maximum length.
...
- **Motion:** one line (`nowrap`). The band only moves when the feed is wider than the lane (user decision 2026-09-26): with a few short actions that fit, it stands still with the newest item on the left and a new action simply takes its place — motion only where it earns its place. When it overflows, it is rendered twice back to back and moved right to left with `atlasTicker` (`translateX(0 → -50%)`, linear, infinite) — exactly one copy per loop, so it wraps without a seam. Items keep their own width and 12px gap; each copy takes at least the lane width (`--ticker-lane`, the measured band width), which keeps the second copy out of view while the band stands still. A moving feed repeats straight after its tail. The duration follows the measured width of one copy at `tickerSpeedPxPerS` (85px/s), so a longer feed or a larger text size reads at the same speed. A new action (or a siege turning into a conquest) restarts the band: when it moves, it enters from the right edge with `atlasTickerIn` (`translateX(lane → 0)`, same speed) — the same way it leaves on the left, never popping in (user decision 2026-09-26) — and then continues seamlessly into the loop; an item that only updates in place (the next army on the same territory) keeps the band running. Transform only — no layout property animates. Under `prefers-reduced-motion` the band stands still with the newest item in view. The second copy is `aria-hidden`.
