...
### TV Pairing (`TvPairPage` / `HomePage`, "TV opzetten" / "TV koppelen")
Added 2026-09-25 (FO §2.2, **TV opzetten**). Written by hand, pending a `/impeccable document` regeneration — it introduces no new values, only reuses existing ones.
- **TV — pairing screen (`/tv`):** the TV lobby's layout with a different rail. `TvWaitingLayout` is the shared title-left / rail-right split with its own left/right wash (`lobbyPanelScrim`) that the lobby already had: `TvTitleColumn` on the left (glass kicker "TV koppelen", the OPERATIE ATLAS title, the waiting line with the pulsing `waitingDot`), and in the rail a single `QrCodePanel`, the same QR block as the lobby's join panel (`LobbyQrPanel` is now a thin wrapper around it) — white QR tile, title "Scan met de telefoon van de host", the URL written out, and the pairing code in the pitch-solid chip. No player list or settings: there is no game yet (**The Invisible Design Rule**). The waiting line doubles as status: "Verbinden…" while there is no code (the rail stays empty), the waiting message with a pointer to "TV koppelen" once the QR shows, and a "retrying" line when fetching a code failed — the TV has no controls (FO §2.1), so it retries on its own. Once the host sends a game, the TV goes straight to that game's own screens; no transition of its own.
- **Phone — start screen:** four entry cards instead of two, all in the existing card shape (base glass, `border-strong` outline, H2 display title plus a muted one-line description): "Nieuw spel starten", "Deelnemen aan een spel", "TV opzetten" (this device becomes the TV) and "TV koppelen" (type the TV's pairing code instead of scanning).
- **Phone — pairing step (`/pair/:pairingCode`):** the same start screen with a "TV gevonden" glass panel above the cards (H2 display title plus a secondary-color description) and two cards: "Nieuw spel starten" and "Spelcode naar TV sturen". Code entry — the pairing code under "TV koppelen", or the game code to send — reuses the join screen unchanged: one base panel holding the H1 title and the `TextField`, then the `Footer`. The primary button reads "Koppelen" or "Naar TV sturen"; when this phone just created a game whose send failed, a secondary "Naar de lobby" sits above it (secondary over primary, as in every footer), and the error shows above the buttons.
- **Phone — sent:** one centred base panel ("Verstuurd naar de TV" plus the game code in secondary text) with only secondary buttons in the footer — "Naar de lobby" when this phone created a game, and "Naar de startpagina". No primary CTA and no glow: nothing on this screen is the next step in a game.
