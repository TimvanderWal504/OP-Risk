...
public sealed class GameHubLobbyTests(PostgresFixture postgres) : IAsyncLifetime
{
    ...
    public Task InitializeAsync()
    {
        ...
        _factory = ApiTestHost.Create(postgres, loggerProvider: _logs);
    ...
    [Fact]
    public async Task CreateGame_MetOnbekendeMapId_GeeftBadRequestInPlaatsVanOnbehandelde500()
    {
        var response = await _client.PostAsJsonAsync("/games", new CreateGameRequest("geen-bestaande-kaart", Settings));
