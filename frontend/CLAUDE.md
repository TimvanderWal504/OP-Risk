...
| TvMainBoardScreen.tsx, TvClaimingScreen.tsx, TvInitialPlacementScreen.tsx (`ActionTicker`) | De `146px`-onderrij toont het verloop als **doorlopende ticker** (rechts → links, `atlasTicker`, 85px/s; sinds 2026-09-26 alleen als het verloop breder is dan de baan — anders staat het stil — en dan komt een nieuwe band van rechts binnen via `atlasTickerIn`) met hoogstens 10 acties en een "Laatste"-badge op de nieuwste; de spelernaam staat in de glas-tekstkleur, niet in de stoelkleur | (voormalige design-reference-export, verwijderd 2026-08-04 — zie git-historie; "Main board", `feedEls`) toonde op die plek een **stilstaande** feed-strip met 5 items waarin alleen een nieuwe kop inschoof (`feedIn`), met de naam in de stoelkleur | bewuste, door de gebruiker opgedragen visuele wijziging (plan-testronde-tv punt 4, 2026-09-25), vastgelegd in `DESIGN.md` § Action Ticker. Naam in glas-tekst: blauw/paars als tekst op donker glas is op TV-afstand onvoldoende leesbaar, de avatar draagt de stoel al. Vervangt de eerdere rij "Geen feed-strip" — die databron bestaat nu (`recentActions`) |
...
| PlaceReinforcementStep.tsx | Geen "klaar → aanvallen"-knop meer; zodra de server `armiesLeft` op 0 zet eindigt de Versterken-fase automatisch (geen klikbare eindstaat) | (voormalige design-reference-export, verwijderd 2026-08-04 — zie git-historie; `reinfDoneBg`/`reinfDoneLabel`) kende een "klaar"-knopstaat — de demo plaatste lokaal zonder server-round-trip | op verzoek van de gebruiker gewijzigd (2026-08-18, gescreenshot): de "klaar"-staat liet alleen een knop zien zonder verdere keuze — niets om nog te beslissen zodra alle legers bevestigd zijn. De tussenstaat "bevestigen" (lokaal volledig verdeeld, nog niet verstuurd) blijft wél nodig, zie doc-comment `locales/reinforce.ts` (`confirmLabel`) |
...
| TvCombatOverlay.tsx | Rechterspelerpaneel (C12) ontbreekt | (voormalige design-reference-export, verwijderd 2026-08-04 — zie git-historie) toonde 'm in de combat-overlay | geaccepteerde scope-afbakening, geen deliverable in het goedgekeurde Attack-plan. **Bijgewerkt (plan-rollen taak 6, bevinding 2):** deze rij noemde ook een ontbrekende reroll-chip — plan-rollen B2 koos bewust tegen een chip (de dobbelsteen-herwerpanimatie zelf is het signaal, `TvCombatOverlay`'s `CombatSide` verwerkt `kind = "reroll"` sinds taak 6), dus dat deel was geen bevinding meer maar een geaccepteerd, ander ontwerp — niet stilzwijgend blijven staan als "ontbreekt" |
...
| GlassPanel.tsx / index.css, DESIGN.md, .impeccable/design.json | **Herzien 2026-09-22** (bewuste, expliciet opgedragen tv-leesbaarheidswijziging — gebruiker keek voor het eerst op een echte tv-monitor, screenshots gedeeld). `--glass-bg` wordt nu wél gezet, als de bestaande `glassSurface`-tint (alpha's tegelijk opgehoogd: base 0.46→0.72, raised 0.38→0.62, overlay 0.62→0.85). "Clear glass" (deze rij tot en met 2026-08-11 bevestigd als bewuste keuze) bleek op een tv op 3m afstand onvoldoende: blur (16px) + saturate(1.4) lieten de drukke stage-illustratie zelf nog als herkenbare ruis achter tekst staan. Blur tegelijk verhoogd (sm/md/lg: 8/16/28 → 14/32/44) en saturate verlaagd (1.4 → 1.0); on-glass tekstopaciteit 80/60%→90/75% en text-shadow verzwaard (`0 1px 3px .55`→`0 2px 6px .85`), zie index.css. `kickerGlassTint`/`buttonPrimaryGlassTint` bestonden al als afgeleide tokens maar werden nergens gebruikt — nu alsnog gewired op `InstructionKicker`/`Button`/de lobby-badge | Was: geen achtergrondtint, "clear glass" | zichtbaar op elk glaspaneel (TV én telefoon, gedeeld component) — donkerdere, rustiger ondergrond, minder doorschijnend. **Bijgewerkt 2026-09-24:** `DESIGN.md` en de `.impeccable/design.json`-sidecar beschrijven dit getinte glas nu (`/impeccable document`, samen met plan-testronde-tv punt 2) — de spec loopt niet meer achter |
