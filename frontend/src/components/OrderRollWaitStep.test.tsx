...
describe('OrderRollWaitStep', () => {
  it('toont de gooien-knop en dobbelsteen-placeholders zolang er nog niet gegooid is', async () => {
    const onRoll = vi.fn()
    render(<OrderRollWaitStep myDice={undefined} colorHex="#ca3c25" canRoll onRoll={onRoll} />)
  ...
  it('toont de eigen worp in de eigen spelerskleur zodra die binnen is', () => {
    render(<OrderRollWaitStep myDice={[6, 4]} colorHex="#ca3c25" canRoll={false} onRoll={vi.fn()} />)

    const dice = screen.getAllByRole('img', { name: /dobbelsteen/i })
    expect(dice).toHaveLength(2)
    expect(dice[0].style.border).toBe(expectedColorMixBorder('#ca3c25'))
